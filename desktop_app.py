...
def wait_for_gateway(url: str = "http://127.0.0.1:8088/", timeout_sec: int = 10) -> bool:
...
def main():
    ...
    if not is_port_in_use(8088):
        if not GATEWAY_EXE.exists():
            fallback_exe = BASE_DIR / "core-gateway" / "target" / "release" / "core-gateway.exe"
            if fallback_exe.exists():
                exe_to_run = fallback_exe
            else:
                exe_to_run = None
        else:
            exe_to_run = GATEWAY_EXE

        if exe_to_run and exe_to_run.exists():
            print(f"[*] Menyalakan ZentyElastis Core Gateway ({exe_to_run})...")
    ...
    # 3. Jalankan Edge Telemetry Agent jika ada script
    ...
    # 4. Buka Native Desktop Window dengan pywebview
    import webview
    ...
    try:
        ...
        # 5. Shutdown bersih saat aplikasi desktop ditutup pengguna
        print("[*] Membersihkan proses di latar belakang...")
        if agent_proc:
            try:
